...
void Player::playerMovement(){
    ...
    if (IsKeyDown(KEY_A) || IsKeyDown(KEY_LEFT) || !is_b_pressed(3)){
        oldLocation = currLocation;
    ...
    else if (IsKeyDown(KEY_W) || IsKeyDown(KEY_UP) || !is_b_pressed(1)){
        oldLocation = currLocation;
        currLocation.y -= speed;
        framesCounter += speed; 
        currMov = up;
    }
    else if (IsKeyDown(KEY_S) || IsKeyDown(KEY_DOWN) || !is_b_pressed(2)){
        oldLocation = currLocation;
        currLocation.y += speed;
        framesCounter += speed; 
        currMov = down;
    }
    else if (IsKeyDown(KEY_D) || IsKeyDown(KEY_RIGHT) || !is_b_pressed(0)){
        oldLocation = currLocation;
        currLocation.x += speed;
        framesCounter += speed; 
        currMov = right;
