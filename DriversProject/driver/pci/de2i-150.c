...
/* meta information */
//Define informações sobre o módulo, como a licença, autor e descrição.
MODULE_LICENSE("GPL");
...
/*Essa função é usada para direcionar o fluxo de controle dentro do driver com base nos comandos IOCTL
 recebidos do espaço do usuário, definindo ponteiros para diferentes regiões de memória do dispositivo
  e índices para diferentes periféricos, permitindo que o driver realize operações apropriadas com base
   na solicitação do usuário.*/
static long int my_ioctl(struct file*, unsigned int cmd, unsigned long arg) 
{	//Chamada quando o user envia um comando de controle para o dispositivo de character associado
	switch(cmd){
	case RD_SWITCHES:
...
/*Essa função é responsável por habilitar o dispositivo PCI, ler informações de configuração do dispositivo, 
reservar e mapear a região de memória BAR 0 e inicializar ponteiros de leitura e escrita para operações futuras no dispositivo.*/
static int __init my_pci_probe(struct pci_dev *dev, const struct pci_device_id *id)
{
	unsigned short vendor, device;
	unsigned char rev;
	unsigned int bar_value; 
	unsigned long bar_len;	//Tamanho do Base Address value

	/* enable the device */
	//Verifica se o dispositivo PCI está habilitado
	...
	//Lê as informações do dispositivo PCI para preencher as variáveis correspondentes
	...
	// Lê as informações do registrador de endereço base(BAR)0 do dispositivo PCI e calcula o tamanho da região de memória mapeada pelo BAR
	...
	// Tenta reservar a região de memória mapeada pelo BAR
	...
	//Mapeia a região de memória física do BAR0 para o espaço de endereço virtual do kernel
	bar0_mmio = pci_iomap(dev, 0, bar_len);

	/* initialize a default peripheral read and write pointer */
	//Incializa os ponteiros de leitura e escrita pra apontar para as regiões de memória mapeada pelo bar0_mmio
	write_pointer = bar0_mmio + 0xC000; //TODO: update offset
	read_pointer  = bar0_mmio + 0xC080; //TODO: update offset
...
static void __exit my_pci_remove(struct pci_dev *dev)
{
	//Indica que não há mais regiões de memória associada a leitura e escrita
	read_pointer = NULL;
	write_pointer = NULL;

	/* remove the IO mapping done in probe func */
	//Desfaz o mapeamento da região de memória física do BAR 0 que foi mapeada para o espaço de endereço virtual do kernel
	pci_iounmap(dev, bar0_mmio);

	/* disable the PCI device */
	//Desabilita o dispositivo PCI pra indicar so kernel que o dispositivo não está mais em uso e pode ser desconectado
	pci_disable_device(dev);

	/* unmark device PCI BAR0 as reserved */
	//Libera a região de memória do BAR0 que foi reservada em my_pci_probe
